...
DELETE FROM `cockatrice_replays` WHERE `id_game` IS NULL OR `id_game` NOT IN (SELECT `id_game` FROM `cockatrice_replays_access`) LIMIT 10000;
...
SQL;
        ...
        foreach($exceedList as $user) {
            ...
            ReplayAccess::findByAccount($user['id'])->orderByAsc('id_game')->delete()->limit($diff)->execute();
        }
    }
...
SQL;
        ...
        foreach($exceedList as $user) {
            ...
            Deck::findByAccount($user['id'])->orderByDesc('id')->delete()->limit($diff)->execute();
        }
    }
    ...
    public function cmdDeleteAccounts() {
        ...
        Account::find()->where(['active', 0])->orderByDesc('id')->delete();
    }
